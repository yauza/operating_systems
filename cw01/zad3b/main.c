...
int main(int argc, char **argv){
...
#ifdef DYNAMIC
    void* lib_handle = dlopen("./liblib.so", RTLD_LAZY);
    struct MainArray (*create_main_array)(int) = dlsym(lib_handle, "create_main_array");
    char* (*merge_files)(char*, char*, int) = dlsym(lib_handle, "merge_files");
    void (*remove_block)(struct MainArray, int) = dlsym(lib_handle, "remove_block");
    void (*remove_verse)(struct MainArray, int, int) = dlsym(lib_handle, "remove_verse");
    char** (*get_sequence)(char*) = dlsym(lib_handle, "get_sequence");
    void (*add_block_pointer)(char*, struct Block*) = dlsym(lib_handle, "add_block_pointer");
    int (*check_if_integer)(char*) = dlsym(lib_handle, "check_if_integer");
    void (*print_main_array)(struct MainArray) = dlsym(lib_handle, "print_main_array");
#endif
    ...
    int main_array_created = 0, block_number = 0;

    struct MainArray mainArray;
    time_report = fopen("raport3b.txt", "a");
